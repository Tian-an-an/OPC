...
export function FrontLogin() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (username === settings.cashierUsername && hashedPassword === settings.cashierPass) {
      toast.success("登录成功");
      navigate("/pos");
    } else {
      toast.error("账号或密码错误");
    ...
    <div className="relative flex h-screen w-full items-center justify-center overflow-hidden bg-gray-100">
      ...
      <div className="z-10 w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl">
        <div className="mb-8 text-center">
          <h1 className="text-3xl font-bold text-gray-800">{settings.storeName}</h1>
          <p className="mt-2 text-gray-500">{settings.cashierTitle}</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div>
            ...
            <input
              ...
              className="mt-1 block w-full rounded-lg border border-gray-300 px-4 py-3 shadow-sm focus:border-blue-500 focus:ring-blue-500 outline-none transition-all"
              placeholder="请输入账号"
            />
          </div>
          <div>
